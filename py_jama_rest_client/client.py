...
class JamaClient:
    ...
    __allowed_results_per_page = 50  # Default is 20, Max is 50. if set to greater than 50, only 50 will items return.
    ...
    def __init__(self, host_domain,
        ...
        self.__enable_para_request = enable_para_request
        self.__max_para_request = max_para_request
    ...
    def get_baselines(self, project_id, allowed_results_per_page=None):
        """
    ...
    def get_baselines_versioneditems(self, baseline_id, allowed_results_per_page=None):
        """
    ...
    def get_baselines_versioneditems_versionedrelationships(self, baseline_id, item_id, allowed_results_per_page=None):
        """
        Get all versioned relationships for the item in the baselin

        Args:
            baseline_id: The id of the baseline to fetch items for.
            item_id: The id of the item to get relationships

        Returns: A list of relationships belonging to the item.

        """
        if allowed_results_per_page == None:
            allowed_results_per_page = self.__allowed_results_per_page

        resource_path = 'baselines/' + str(baseline_id) + '/versioneditems/' + str(item_id) + '/versionedrelationships' 
        try:
            response = self.__core.get(resource_path)
        except CoreException as err:
            py_jama_rest_client_logger.error(err)
    ...
    def get_projects(self, allowed_results_per_page=None):
    ...
    def get_project(self, project_id, allowed_results_per_page=None):
        """
        This method will return a singular project of a specified project id
        Args:
            project_id: the project id of the project to fetch

        Returns: a dictonary object representing the project
    ...
    def get_filter(self, filter_id):
        """
        This method will return a singular filter of a specified filter id
        Args:
            filter_id: the filter id of the filter to fetch

        Returns: a dictonary object representing the filter

        """
        resource_path = 'filters/' + str(filter_id)
        try:
            response = self.__core.get(resource_path)
        except CoreException as err:
            py_jama_rest_client_logger.error(err)
    ...
    def get_filters(self, project_id=None, author_id=None, filterScope=None, accessSpecifier=None, allowed_results_per_page=None):
        """
        Get all filters with the specified conditions

        Args:
            project_id: Project id beloging the filters
            author_id: User id of filter creator
            filterScope: Filter scope. More than one scope can be selected. 
                         Available values : ALL_PROJECTS, BOUND_PROJECT, CURRENT_PROJECT
            accessSpecifier: Available values : PUBLIC, PRIVATE, PUBLIC_AND_PRIVATE
            allowed_results_per_page: Number of results per page
        ...
        if accessSpecifier:
            params['accessSpecifier'] = accessSpecifier

        project_data = self.__get_all(resource_path, params=params, allowed_results_per_page=allowed_results_per_page)
        return project_data
    
    def get_filter_results(self, filter_id, project_id=None, allowed_results_per_page=None):
        """
    ...
    def get_items(self, project_id, rootOnly="false", allowed_results_per_page=None):
        """
        ...
        if allowed_results_per_page == None:
            allowed_results_per_page = self.__allowed_results_per_page
        resource_path = 'items'
        params = {'project': project_id, 'rootOnly': rootOnly}
    ...
    def get_item_tags(self, item_id, allowed_results_per_page=None):
        """
    ...
    def get_abstract_items_from_doc_key(self, doc_key_list, allowed_results_per_page=None):
    ...
    def get_relationship_types(self, allowed_results_per_page=None):
        """
    ...
    def get_item_types(self, allowed_results_per_page=None):
        """
    ...
    def get_items_synceditems(self, item_id, allowed_results_per_page=None):
        """
    ...
    def get_item_versions(self, item_id, allowed_results_per_page=None):
        """
        Get all versions for the item with the specified ID
    ...
    def get_pick_lists(self, allowed_results_per_page=None):
        """
    ...
    def get_pick_list_options(self, pick_list_id, allowed_results_per_page=None):
        """
    ...
    def get_relationships(self, project_id, allowed_results_per_page=None):
        """
    ...
    def get_item_children(self, item_id, allowed_results_per_page=None):
        """
    ...
    def get_items_upstream_relationships(self, item_id, allowed_results_per_page=None):
        """
    ...
    def get_items_downstream_related(self, item_id, allowed_results_per_page=None):
        """
    ...
    def get_items_downstream_relationships(self, item_id, allowed_results_per_page=None):
        """
    ...
    def get_tags(self, project, allowed_results_per_page=None):
        """
    ...
    def get_tagged_items(self, tag_id, allowed_results_per_page=None):
        """
    ...
    def get_users(self, allowed_results_per_page=None):
        """
    ...
    def get_activities(self, projects, eventType=None, objectType=None, date=None, allowed_results_per_page=None):
        """
        This method will return all activities in the project with the specified ID.
        Args:
            projects: project id
            eventType: Event type to filter on. More than one event type can be chosen
            objectType: Object type to filter on. More than one object type can be chosen
            date: Filter datetime fields after a single date or within a range of values. Provide one or two values in ISO8601 format (milliseconds or seconds) - "yyyy-MM-dd'T'HH:mm:ss.SSSZ" or "yyyy-MM-dd'T'HH:mm:ssZ"
            allowed_results_per_page: Number of results per page
        Returns: An array of dictionary objects
        """
        if allowed_results_per_page == None:
        ...
        params = {'project': projects, 'eventType':eventType,'objectType':objectType, 'date': date}
        users = self.__get_all(resource_path,   params=params, allowed_results_per_page=allowed_results_per_page)
        return users

    def get_activities_affecteditems(self, activity_id, allowed_results_per_page=None):
        """
        This method will return all items affected by the activity with the specified ID.
        Args:
        ...
        if allowed_results_per_page == None:
            allowed_results_per_page = self.__allowed_results_per_page
        resource_path = 'activities/%d/affecteditems' % activity_id
        users = self.__get_all(resource_path,  allowed_results_per_page=allowed_results_per_page)
        return users

    def get_item_attachments(self, item_id , allowed_results_per_page=None):
        """
        This method will return all attachments in the specified item.
        Args:
            item_id: item id to which the attachment is attached
            allowed_results_per_page: Number of results per page
        Returns: An array of dictionary objects
        """
        if allowed_results_per_page == None:
        ...
        item_attachments = self.__get_all(resource_path,  allowed_results_per_page=allowed_results_per_page)
        return item_attachments

    def get_attachments_file(self, attachment_id):
        """
        This method will return content of attachment file.
        Args:
    ...
    def post_project(self, key, name,is_folder=False, parent_id=None):
        """
        Create a new project or project folder in Jama Connect.
        Args:
            key: The key for the new project or folder.
            name: The name for the new project or folder.
    ...
    def post_item_type(self, key, display,displayPlural, description="", image="", widgets=[],category=None):
        ...
        Args:
            key: The key for the new item type.
        ...
        """
        # Process widgets to replace "HISTORY" with "VERSIONS"
        processed_widgets = []
        for widget in widgets:
            if isinstance(widget, dict) and widget.get('name') == 'HISTORY':
                widget = widget.copy()  # Create a copy to avoid modifying the original
                widget['name'] = 'VERSIONS'
    ...
    def __get_all(self, resource, params=None, allowed_results_per_page=None, **kwargs):
        ...
        limit_allowed_results_per_page = 10000 if resource == "relationships" else 50
        if allowed_results_per_page < 1 or allowed_results_per_page > limit_allowed_results_per_page:
            raise ValueError(f"Allowed results per page must be between 1 and {limit_allowed_results_per_page}")

        data = []

        if self.__enable_para_request:
            page_response = self.__get_page(resource, 0, params=params, allowed_results_per_page=allowed_results_per_page, **kwargs)
            page_json = page_response.json()
            page_info = page_json['meta']['pageInfo']
            data.extend(page_json.get('data'))
            
            if page_info.get('totalResults') > allowed_results_per_page:
                from concurrent import futures
                with futures.ThreadPoolExecutor(max_workers=self.__max_para_request) as executor:
                    for idx in range(allowed_results_per_page, page_info.get('totalResults'), allowed_results_per_page):
                        future = executor.submit(self.__get_page, resource, idx,  params=params, allowed_results_per_page=allowed_results_per_page)
                        future.add_done_callback(lambda future: data.extend(future.result().json().get('data')))
        else:
            start_index = 0
            # allowed_results_per_page = 20
    ...
    def __get_page(self, resource, start_at, params=None,  allowed_results_per_page=None,  **kwargs):
